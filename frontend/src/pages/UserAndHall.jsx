...
function UserAndHall() {
    ...
    useEffect(() => {
        axios
            .get(`${api_link}/user-and-hall/${params.user_id}`)
            .then((res) => {
                const tempHalls = [];
                for (let i = 0; i < res.data.length; i++) {
                    const hall = res.data[i].hall;
                    tempHalls.push(hall);
                    locations.add(res.data[i].hall.location);
                    res.data[i].hall.sports.forEach((sport) => sports.add(sport));
                }
                setIsLoading(false);
                setHalls(tempHalls);
            })
            .catch((err) => {
                ...
                if (!alertShownRef.current) { 
                    alert("Ovaj korisnik nema oglasa. Bit ćete preusmjereni na profil.");
                    alertShownRef.current = true; 
                    window.location.href = `/users2/${params.user_id}`; 
                }
            });
    }, [params.user_id, api_link]);
