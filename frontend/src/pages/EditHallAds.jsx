...
export default function EditHallAds() {
    const [token, ] = useContext(UserContext)
    const user = jwtDecode(token)
    const [showSportsPopUp, setShowSportsPopUp] = useState(false)

    const { id } = useParams();
    const current_hall_id = parseInt(id);
        ...
        || adsData.price == ""
            ...
            try {
               ...
               if (selectedSports.length === 0) {
                ...
                const AddHallResponse = await axios.put("http://localhost:8000/update-hall/" + current_hall_id, {
                ...
                console.log("Success, added ad updated: ", AddHallResponse.data);
                toast.success("Uspjesno ste uredili oglas za teren!", options);
                setTimeout(() => {
                    window.location.href = "/tereni/" + current_hall_id;
                }, 2000);
            } catch (error) {
  ...
  useEffect(() => { // getting all cities to use them in form
    const fetchData = async () => {
      try {
        ...
        const currentData = await fetch('http://localhost:8000/get-one-hall/' + current_hall_id);
        
        if (!response.ok || !responseSports.ok || !currentData.ok) {
          throw new Error('Network response was not ok');
        }
        const data = await response.json();
        const sportsData = await responseSports.json();
        const currentDataJson = await currentData.json();
        const hallDetails = currentDataJson.hall[0].hall;
        
        setCities(data);
        let modifiedSports = sportsData.map((sport) => ({ // adding additional property selected to each sport
            ...sport,
            selected: false
        }))
        ...
        setAdsData({
            ...
            id_city: hallDetails.id_city, // default value is Sarajevo
            address: hallDetails.address,
            start_of_working: hallDetails.start_of_working,
            end_of_working: hallDetails.end_of_working,
            description: hallDetails.description,
            no_of_players: hallDetails.no_of_players,
            dimensions_width: hallDetails.dimensions_width,
            dimensions_height: hallDetails.dimensions_height,
            price: hallDetails.price
        });
        
        const alreadySelectedSports = hallDetails.sports_associations.map(sport => sport.id_sport);
        setSports(prevSports => prevSports.map(sport => ({ ...sport, selected: alreadySelectedSports.includes(sport.id_sport) })));

        setPhotos([]);
        hallDetails.photos.map(photo => setPhotos(prevPhotos => [...prevPhotos, photo.photo]));
  ...
  const handleRemovePhoto = (photoToRemove) => { // remove photo from gallery
    ...
    return (
        <div className="hall-ads">
            ...
            <h2 className="mb-10">Uredi oglas za teren</h2>
